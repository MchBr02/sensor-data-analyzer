...
/**
 * Handle and save JSON data. If the data contains valid payloads, save them to collections.
 */
/*
async function saveDataToDatabase(req: Request, timestamp: string): Promise<void> {
  try {
    const data = await req.json();
    // Check for valid payloads
    if (
      "deviceId" in data &&
      ...
      for (const item of data.payload) {
        if (
          ...
          // Save to collection using the payload name
          const collectionName = item.name;
          const databaseName = data.deviceId;
          await saveToCollection(item, collectionName, databaseName);
          // console.log(`✅ Saved payload to collection "${collectionName}"`);
        } else {
          console.warn("❌ Invalid or empty payload:", item);
        }
      }
    ...
    // Save the full data to a file regardless
    // const dataFileName = `${dataFolder}/sensor-data-${timestamp}.json`;
    // await Deno.writeTextFile(dataFileName, JSON.stringify(data, null, 2));
    // console.log(`✅ Full data saved to: ${dataFileName}`);
  } catch (error) {
    let errorMessage = "❌ Error handling request data:";
    if (error instanceof Error) {
        errorMessage = error.message;
...
/**
 * Handle the request headers data
 */
/*
async function handleHeadersData(req: Request, timestamp: string, dataFileName: string): Promise<string> {
  const metadataFileName = `${dataFolder}/sensor-data-${timestamp}-requestHeaderData.json`;

  // Collect all headers in a key-value pair object
  ...
  await Deno.writeTextFile(metadataFileName, JSON.stringify(metadata, null, 2));
 ...
 */
/*
export async function RequestHandler(req: Request): Promise<void> {
  try {
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    await saveRequestData(req, timestamp);
    console.log("✅ Request data handled successfully");
  } catch (error) {
    let errorMessage = "❌ Error handling request data:";
    if (error instanceof Error) {
    ...
    throw new Error("Failed to process the request data");
  }
}
*/

/*
export async function saveJsonData(data: Record<string, unknown>) {
  try {
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const baseFileName = `${dataFolder}/sensor-data-${timestamp}`;
    ...
    console.log(`✅ Data saved successfully to: ${dataFileName}`);
    console.log(`✅ Metadata saved successfully to: ${metadataFileName}`);
  } catch (error) {
    let errorMessage = "Failed to save sensor data";
    if (error instanceof Error) {
        errorMessage = error.message;
    }
    console.error("❌ Failed to save sensor data:", errorMessage);
